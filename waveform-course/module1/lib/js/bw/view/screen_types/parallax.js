...
function Parallax(s)
{
	...
	self.custom = function(custom_xml_node)
	{
		...
		this.width = self.custom_xml.attr('width');
		this.height = self.custom_xml.attr('height');
		this.x = self.custom_xml.attr('x');
		this.y = self.custom_xml.attr('y');
		...
		self.$customdiv.append(parallaxHolder);
		...
		for ( var i = 0; i < self.custom_xml.children().length; i++)
		{
			...
			var next_xml_node = self.custom_xml.children() [ i+1 ] ;
	...
	{
		if ( devmode ) console.trace("ENABLE PARALLAX:")
		var draggable = Draggable.get(self.scrubLayer.layerDiv);
		if( draggable )
		{
			draggable.enable();
	...
	this.attachContents = function()
	{
		...
		for( var i = 0; i < this.layerArray.length; i++ )
		{
			...
			this.createScreenElements(oLayer.layer_node);
		}
	}
