...
{		
	...
	{
		var start_div_id = self.idprefix + self.model.id + '_start';
		self.start_div = $( '<div class="start" id="' + start_div_id + '">' );
		self.$content.append( self.start_div );
		
		var end_div_id = self.idprefix + self.model.id + '_end';
		self.end_div = $( '<div class="end" id="' + end_div_id + '">' );
		self.$content.append( self.end_div );
	...
	self.createController = function(  )
	{
		...
		if( xml_node )
		{
			...
			else
			{
				...
				self.$content.prepend( self.toggle );
