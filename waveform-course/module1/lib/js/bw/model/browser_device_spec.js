...
function BrowserDeviceSpec( argDeviceSpec , argDesktopBrowserSpec , hasVideo, responsiveMode ) {	
	...
	for (i=0;i<deviceSpecArray.length;i++) {					
		...
		switch (specCode) {
			...
			default: {
				console.warn("unknown device. Please check devicespec in master.xml")
			}
		}
	}	
	console.log("self.deviceSpec = %O",self.deviceSpec)
	...
	for (i=0;i<desktopBrowserSpecArray.length;i++) {					
		...
		switch (specCode) {
			...
			default: {
				console.warn("unknown desktop browser. Please check desktopbrowserspec in master.xml")
			}
		}
	}
	console.log("self.desktopBrowserSpec = %O",self.desktopBrowserSpec)
	...
	self.checkDeviceSpec = function () {
		...
		if ( !passedDeviceChecks ) {			
			...
			console.log("passed device checks!\n");
		}				
		...
		*/
		...
		if ( !passedBrowserChecks ) {	
			if (isIECompatibilityModeError) {
				console.warn("ieCompatibilityModeError");		
			...
			passedChecks = true;
			console.log("passed windows desktop browser checks!\n");
		}				
		return passedChecks;
	}
	
	self.doErrorChecks = function () {
		console.log("doErrorChecks");
	...
	self.screenOrientationError = function() {
		...
		console.warn("screenOrientationError\n");
