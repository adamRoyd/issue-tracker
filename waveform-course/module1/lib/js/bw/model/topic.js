...
function Topic(xml_node) 
{
	...
	self.sDuration = self.xml.attr("duration") || "";
	self.topicClass = self.xml.attr("class") || "";
	
	self.attempts = 0;
	self.assessment = Boolean(self.xml.attr("assessment")=="true");
	self.locked = Boolean(self.xml.attr("locked")=="true");
	self.masteryScore = Number(self.xml.attr("masteryscore"));
	self.iBestScore = 0;
	...
	self.required = true ;
	if( self.xml.attr( "required" ) )
	{
		self.required = Boolean( self.xml.attr( "required" ) == "true" ) ;
	}
	...
	self.updateStatus = function()
	{
		if( self.locked )
		{
			...
			//console.log("UNLOCKING check: " + self.id ) ;
			var areRequiredTopicsUnlocked = masterObj.areTopicsUnlocked( self.xml.attr("unlock") );
			if( areRequiredTopicsUnlocked )
			{
				self.unlock();
			}
			else
			{
				...
				//console.log("	unlock requirement for %s is: %s", self.id, all_completed ) ;
		...
		if( self.arScreens.length >0 )
		{
			...
			if( ( visited_count > 0 ) && ( self.getStatus() != "completed" ))
			{
				self.setStatus( "incomplete" );
				self.attempted = true;
			}
			...
			{
				if( self.iBestScore >= self.masteryScore )
				{
					self.setStatus("passed" );
						self.completed = true;
	...
	self.getPassed = function( )
	{
		...
		if( self.getAssessment() )
		{
			if( self.getBestScore() >= self.masteryScore )
			{
				passed = true;
			}
		}
