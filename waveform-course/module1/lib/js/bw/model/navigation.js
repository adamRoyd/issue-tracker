...
function Nav()
{
	...
	//	self.createPageCounter() ;
		tweenTo(self.$div, 0, {autoAlpha:1, display:'block'});
		//self.createLoader() ;
	}
	// --------------------------------------  L O A D E R -------------------------------------- //
	...
	self.createLoader = function()
	{
		...
		self.$div.append(self.loadergraphic);
	};
	...
	// {		
		...
		// self.navScreen.view.$div.append(self.pagecounter.view);	
	...
	self.initNav = function()
	{
		...
		if( currentScreen )
		{
			...
			self.courseHolder.addClass( currentTopic.topicClass );
	...
	{
		tweenTo(self.$div, 0.3, {autoAlpha:1, display:'block'});
	};
	...
	{	
		var holderdiv = currentScreen.view.$div;
		// if( previousScreen )
		// {
	...
	self.showDialogueWindow = function()
	{
		...
		if( _respondo.desktop() )
		{
			...
			//tweenTo($('#dialogueholder'), 0, {autoAlpha:0, left: 0, marginLeft: 0, top: '50%', marginTop: 0 - (currentDialogue.height/2) });
	...
	self.initDialogueWindow = function()
	{
		...
		tweenTo(currentDialogue.view.$div, 0, {autoAlpha:1 , onComplete: currentDialogue.view.ready, display:'block'});
		
		if( _respondo.phone() )
		{
			// disable other layers to prevent multi-scroll on phones
			$('#contentholder').addClass( 'hidden' ) ;
			$('#menuholder').addClass( 'hidden' ) ;
			$('#navholder').addClass( 'hidden' ) ;
			$('#dialoguemask').addClass( 'hidden' ) ;
		}
	
		currentDialogue.finaliseDisplay() ;
	};
	...
	self.closeDialogue = function()
	{
		...
		{
			if ( devmode ) console.log( 'LOG: NO CURRENT SCREEN' );
		}
	};
