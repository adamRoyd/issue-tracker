...
function applyClick(elm, func, overfunc, outfunc) 
{
	...
	btn.click(function(){
		...
		if( !elm.model || !elm.model.doubleTap )
...
function enableContentButton(elm)
{
	if( elm )
	{
		//elm.view.$div.removeClass('disabled');
		elm.enable();
	}
}
...
function disableContentButton(elm)
{
	if( elm )
	{
		//elm.view.$div.addClass('disabled');
		elm.disable();
	}
}
...
function openGlossaryAtTerm( term )
{
	//console.log("OPEN GLOSSARY at term: " + term ) ;
	openDialogue( 'glossary', [term] ) ;
}
