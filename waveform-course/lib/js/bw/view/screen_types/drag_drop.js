...
function Drag_drop(s)
{
	...
	self.resetScreen = function()
	{
		...
		for( var i = 0; i < self.dragArray.length; i++ )
		{
			...
			oDrag.drag.removeClass('in_target')
	...
	self.drag = function( oXML, id )
	{
		...
		drag_obj.dropevent 		= oXML.attr( "event" );
		...
		if( element )
		{
			var dragElement 		= element.view.$div;		
			...
			drag_obj.resetPosition = function(  )
			{
				...
				drag_obj.targetDrop	= null ;
				drag_obj.inTarget		= false ;
				drag_obj.drag.removeClass('in_target')
				
				self.doClickById('drop_out');
				...
				onDragEnd : function(e) 
				{
					...
					while (--i > -1) 
					{
						...
						if( isFunction( this.hitTest ) )
						{
							...
							if ( hit ) 
							{
								self.doClickById('drop_in,'+'drop_in_'+drag_obj.id);
								
								if( drag_obj.dropevent )
								{
									self.doClickById(drag_obj.dropevent);
								}
								
								//onDrop(this.target,  self.dropArray[i]);
								drag_obj.targetDrop = self.dropArray[i];
								drag_obj.inTarget = true;
								drag_obj.drag.addClass('in_target')
							}
						}
					}
	...
	{
		self.doClickById('drag');
		if(  drag_object.inTarget ) 
		{
			self.setDragStart( drag_object ) ;
			
		}
		drag_object.drag.addClass('dragging');
	...
	self.setDragStart = function( drag_object )
	{
		...
		drag_object.drag.removeClass('in_target')
	...
	self.dragEnd = function( drag_object )
	{
		...
		drag_object.drag.emoveClass('dragging');
		for( var i = 0; i < self.dropArray.length; i++ )
		{
	...
	{
		drag_object.drag.removeClass('dragging');
		if( drag_object.inTarget )
		{
	...
	self.drop = function( oXML )
	{
		...
		var elementStr = oXML.text() ;
		var dropElement 		= self.screen.view.getScreenElementById( elementStr ).view.$div;		
				...
				if( drop_obj.maxdrop > 1 )
				{
					for( var i = 0; i < drop_obj.dragArray.length; i++ )
					{
						...
						if( i > 0 )
						{
							...
							var drag_height = prev_item.drag.outerHeight() * self.dropscale
	...
	{
		self.doClickById('correctanswers')
		for( var i = 0; i < self.dragArray.length; i++ )
		{
			var dragObj = self.dragArray[ i ];
			dragObj.inTarget = false;
			dragObj.drag.removeClass('in_target')
		}
...
Drag_drop.prototype = ScreenView;
