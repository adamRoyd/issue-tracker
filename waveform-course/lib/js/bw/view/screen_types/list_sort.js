...
function List_sort(s)
{
	...
	self.appendOptions = function()
	{
		...
		for( var i = 0; i < self.optionArray.length; i++ )
		{
			...
			self.$customdiv.append( oOpt.option );
	...
	{
		self.$customdiv.find('.list_sort_item').sort(self.y_sort).appendTo(self.$customdiv);

		TweenMax.to($('.inner'), 0, {y:0})
		self.$customdiv.find('.dragging').removeClass('dragging')
	}
	...
	self.submit = function()
	{
		...
		var option_elements = self.$customdiv.find( '.list_sort_item' );
	...
	{
		self.$customdiv.find('.list_sort_item').sort(self.correct_sort).appendTo(self.$customdiv);
	...
	self.reset = function()
	{
		...
		self.$customdiv.find('.list_sort_item').sort(self.original_sort).appendTo(self.$customdiv);
...
List_sort.prototype = ScreenView;
