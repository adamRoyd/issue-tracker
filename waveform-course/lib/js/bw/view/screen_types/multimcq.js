...
{		
	...
	self.onComplete = function()
	{
		...
		var revealPart = self.view.getScreenElementById('partFinal') || self.view.getScreenElementById('part' + (self.subScreenArray.length - 1));
	...
	self.playScreenAudio = function()
	{
		var screen = self.subScreenArray[self.currentScreenIndex];
		if (!screen)
		{
			return;
		}

		var audioEvent = self.isPassState? 'playSuccessAudio' : 'playScreenAudio';
		screen.view.subScreenObject.doClickById(audioEvent);
	}
