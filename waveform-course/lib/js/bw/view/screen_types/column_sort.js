...
{		
	...
	self.createSettings = function(xml_node)
	{
		...
		self.maxSelections = xml_node.attr( 'maxselections' )? Number( xml_node.attr( 'maxselections' )) : 0;
	...
	self.resetOptions = function(forceReset)
	{
		...
		{
			var row = self.rowArray[i];
			if (self.radiomode && !forceReset && (self.correctRemain && row.isCorrect))
				continue;
			...
			for(var j = 0; j<row.rowOptions.length; j++)
			{
				...
				if( !forceReset && self.correctRemain)
				{
					// If option is correct and selelcted
					if (inArray( option.id , row.correctColumn ) && option.data( 'selected' ))
						continue;
				}				
	...
	self.selectOption = function( radio )
	{
		...
		{
			if (self.maxSelections > 0 && self.selectedCount == self.maxSelections)
			{
				return;
			}
