...
function AudioPlayerView( m )
{
	...
	{
		// Don't load the resource if it's in the soundbank
		if (soundbankObj.hasSound(self.src_url)) 
		{
			if (devmode) console.log("AudioPlayerView: using soundbank for sound - " + self.src_url);
			self.useSoundbank = true;
			var sound = soundbankObj.getSound(self.src_url);
			self.jplayer_div = sound.view.jplayer_div;
			self.setReady();
			return;
		}		
		...
		self.jplayer_skin = $( self.$div.find( ".jp-flat-audio" )[0] );
	...
	self.initPlayback = function(  )
	{
		if (devmode) console.log("initplayback");
		if(!isIE())// should really say if iOS()
		{
			self.jplayer_div.jPlayer( 'volume', 0 );
			self.jplayer_div.jPlayer( 'play' );
			self.jplayer_div.jPlayer( 'stop' );
			self.jplayer_div.jPlayer( 'volume', 1 );
		}
	};		
