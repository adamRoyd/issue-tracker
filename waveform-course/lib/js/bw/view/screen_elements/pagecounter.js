function PagecounterView( m )
{
	...
	self.allow_jump = self.xml_node.attr('jump') == "true";
	self.$div.addClass('pagecounter');
	...
	{
		self.$div.find('.from').empty().append( String( self.current_page_number ) );
		self.$div.find('.to').empty().append( String( self.total_page_number ) );
	}
