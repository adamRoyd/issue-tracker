...
{		
	...
	self.setContent = function()
	{
		...
		if( self.xml_node.attr( 'tabwidth' ) )
		{
			self.default_tab_width = parseNumberString( self.xml_node.attr( 'tabwidth' ) );
		}
	...
	self.createClicks = function()
	{
		...
		self.$content.append( self.clickholder );
		...
		for( var i = 0; i < click_xml_nodes.length; i++ )
		{
			...
			var click_width = self.default_tab_width;
			if( item.attr( 'width' ) )
			{
				click_width = parseNumberString( item.attr( 'width' ) );
			}
			
			clickdiv.width( click_width );
	...
	self.createReveals = function()
	{
		...
		self.$content.append( self.revealholder );
