...
*/
/* Dev: This should be a screen element / timed element and the kill function will need to be supered */
...
function Sprite( view, xml )
{
	...
	self.init = function()
	{
		view.$div.append( self.sprite_div );
		if( self.time )
		{
			self.initAnim();
		}
		else
		{
			if(self.sheeturl)
			{
				...
				view.$div.css('background-image', 'url("' + self.sheeturl + '")' );
			}
			else
			{
				//self.sprite_div.addClass( self.name );
				view.$div.addClass( self.name );
			}
		}
	}
	...
	/* This should be a screen element / timed element and the kill function will need to be supered */
	self.kill = function()
	{
		if( self.timeline )
		{
