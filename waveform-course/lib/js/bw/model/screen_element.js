...
function ScreenElement(xml_node, s)
{
	...
	self.setup = function()
	{
		...
		self.doubleTap = Boolean( $( self.xml ).attr( 'doubletap' ) == 'true' );
	...
	self.findFunc = function( funcId )
	{
		...
		{
			if ( devmode ) console.log("			ELEMENT VIEW FUNC FOUND!") ;
			if ( devmode ) console.log("				element_view_func: " + element_view_func) ;
